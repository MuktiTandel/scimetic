...
class AccessSettingScreen extends StatelessWidget {
                  ...
                  onChange: (value) {
                    ...
                    if ( value.contains(AppStrings.superAdmin) ) {
                      ...
                      for (var element in controller.mainList) {
                        if ( element.role!.name!.contains(AppStrings.superAdmin)) {
                      ...
                      controller.userList.clear();
                      for (var element in controller.mainList) {
                        if ( element.role!.name!.contains(AppStrings.companyOwner)) {
                          controller.userList.add(element);
                        }
                      }
                      ...
                      controller.userList.clear();
                      for (var element in controller.mainList) {
                        if ( element.role!.name!.contains(AppStrings.companyAdmin)) {
                          controller.userList.add(element);
                        }
                      }
                    } else {
                      controller.userList.clear();
                      for (var element in controller.mainList) {
                        if ( element.role!.name!.contains(AppStrings.cropTechnician)) {
                          controller.userList.add(element);
                        }
                      }
                    }
  ...
  Widget listWidget({
    ...
    // final password = controller.decrypt(data.password!);

    // AppConst().debug('password => ${password}');
                        ...
                        Row(
                          children: [
                            ...
                            editDeletePopup(editTap: (){
                              ...
                              controller.roleValue.value = data.role!.name ?? "";
                      ...
                      decoration: InputDecoration(
                          ...
                          suffixIcon: Obx(() => GestureDetector(
                            ...
                            child: Padding(
                              ...
                              child: Image.asset(
                                isShow.value == false ? AppImages.visible : AppImages.unVisible,
                                height: 15.h,
                                width: 15.w,
                                color: Get.isDarkMode ? Colors.white : AppColors.subTitleColor,
                              ),
