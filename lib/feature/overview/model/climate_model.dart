...
ClimateModel climateDataFromJson(String str) =>
...
class ClimateModel {
  ...
  factory ClimateModel.fromJson(Map<String, dynamic> json) => ClimateModel(
        climateData: json["climateData"] != null
            ? List<ClimateData>.from(
                json["climateData"].map((x) => ClimateData.fromJson(x)))
            : [],
...
class ClimateData {
  ...
  factory ClimateData.fromJson(Map<String, dynamic> json) => ClimateData(
        result: json["result"] ?? "",
        table: json["table"] ?? 0,
        start: json["_start"] != null
            ? DateTime.parse(json["_start"])
            : DateTime.now(),
        stop: json["_stop"] != null
            ? DateTime.parse(json["_stop"])
            : DateTime.now(),
        time: json["_time"] != null
            ? DateTime.parse(json["_time"])
            : DateTime.now(),
        device: json["device"] ?? "",
