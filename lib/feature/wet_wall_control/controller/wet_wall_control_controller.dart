...
import 'package:scimetic/feature/wet_wall_control/model/wet_wall_model.dart';
...
class WetWallControlController extends GetxController {
  ...
  WetWallModel wetWallModel = WetWallModel();
    ...
    if (token.isNotEmpty) {
      ...
      try {
        ...
        if (apiResponse!.statusCode == 200) {
          ...
          wetWallModel = WetWallModel.fromJson(data);

          if (wetWallModel.wetWallControl != null) {
            WetWallControl wetWallControl = wetWallModel.wetWallControl!;

            isAuto.value = wetWallControl.wetWallAuto ?? false;
            dayTemperatureController.text = wetWallControl.dayTemperature != 0
                ? wetWallControl.dayTemperature!.toString()
                : "";
            dayTemperatureDeadbandController.text =
                wetWallControl.dayTemperatureDeadband != 0
                    ? wetWallControl.dayTemperatureDeadband!.toString()
                    : "";
            nightTemperatureController.text =
                wetWallControl.nightTemperature != 0
                    ? wetWallControl.nightTemperature!.toString()
                    : "";
            nightTemperatureDeadbandController.text =
                wetWallControl.nightTemperatureDeadband != 0
                    ? wetWallControl.nightTemperatureDeadband!.toString()
                    : "";
            daySwitch.value = wetWallControl.daySwitch ?? "";
            dayRelay.value = wetWallControl.dayRelay ?? "";
            nightSwitch.value = wetWallControl.nightSwitch ?? "";
            nightRelay.value = wetWallControl.nightRelay ?? "";
          }
  ...
  Future addWetWallControlData() async {
    ...
    wetWallControlModel.dayTemperature =
        double.parse(dayTemperatureController.text);
    wetWallControlModel.dayTemperatureDeadband =
    ...
    wetWallControlModel.nightTemperature =
        double.parse(nightTemperatureController.text);
    wetWallControlModel.nightTemperatureDeadband =
    ...
    wetWallControlModel.wetWallAuto = isAuto.value;
